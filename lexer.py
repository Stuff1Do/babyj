    ...
    def tokenize(self):
        ...
        while self.current_char is not None:
            if self.current_char.isspace():
                ...
                num, start_col = self.consume_while(lambda c: c.isdigit())
                tokens.append(Token(TokenType.NUMBER, int(num), self.ln, start_col))

            elif self.current_char == '+':
                tokens.append(Token(TokenType.PLUS, '+', self.ln, self.column))
                self._advance()
            elif self.current_char == '-':
                tokens.append(Token(TokenType.MINUS, '-', self.ln, self.column))
                self._advance()
            elif self.current_char == '*':
                tokens.append(Token(TokenType.MULTIPLY, '*', self.ln, self.column))
                self._advance()
            elif self.current_char == '/':
                tokens.append(Token(TokenType.DIVIDE, '/', self.ln, self.column))
                self._advance()
            elif self.current_char == '=':
                tokens.append(Token(TokenType.ASSIGN, '=', self.ln, self.column))
                self._advance()
            elif self.current_char == ';':
